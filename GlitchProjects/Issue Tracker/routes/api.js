...
const CONNECTION_STRING = process.env.DB; //MongoClient.connect(CONNECTION_STRING,{useUnifiedTopology: true}, function(err, db) {});
...
db.collection(project).find(filter).toArray((err, docs) => {
    ...
    .post(function (req, res){
      var project = req.params.project;    
      var body = req.body;
      if (!body.issue_title.replace(/\s/g, '').length 
          || !body.issue_text.replace(/\s/g, '').length 
          || !body.created_by.replace(/\s/g, '').length) {
    ...
    .put(function (req, res){      
      ...
      var body = req.body;
      var _id = body._id
      var filterPut = {_id: ObjectId(_id)}
      ...
      if (Object.keys(updateIssue).length === 0) {
        ...
        MongoClient.connect(CONNECTION_STRING,{useUnifiedTopology: true}, function(err, client) {
                              var db = client.db();
db.collection(project).updateOne(filterPut, {$set: updateIssue}, (err, docs) => {
