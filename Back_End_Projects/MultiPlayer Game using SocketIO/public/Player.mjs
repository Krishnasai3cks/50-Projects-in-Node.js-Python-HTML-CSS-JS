...
class Player {
  ...
  draw(ct,food,playerArray){
    const currdir = Object.keys(this.direction).filter(d=>this.direction[d]);
  ...
  movePlayer(dir, speed) {
    if (dir === 'up') this.y - speed >= canstate.pfminy ? this.y -= speed : this.y -= 0;
    if (dir === 'down') this.y + speed <= canstate.pfmaxy ? this.y += speed : this.y += 0;
    if (dir === 'left') this.x - speed >= canstate.pfminx ? this.x -= speed : this.x -= 0;
    if (dir === 'right') this.x + speed <= canstate.pfmaxx ? this.x += speed : this.x += 0;
  }
  ...
  calculateRank(arr) {
    ...
    const rank = this.score === 0 ? arr.length : (newscores.findIndex(a => a.id === this.id) + 1);
