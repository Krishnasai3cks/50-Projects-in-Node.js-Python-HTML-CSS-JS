...
I can report a reply and change it's reported value to true by sending a PUT request to /api/replies/{board} and 
    pass along the thread_id & reply_id. (Text response will be 'success')
Complete functional tests that wholely test routes and pass.
*/
let expect = require('chai').expect;
let replies = require('../controllers/replies.js');
let threads = require('../controllers/threads.js');

module.exports = function (app) {
  let reply = new replies();
  let thread = new threads();
  app.route('/api/threads/:board').get(thread.getlist).post(thread.newthread).put(thread.reportthread).delete(thread.deletethread);
  app.route('/api/replies/:board').get(reply.getlist).post(reply.newreply).put(reply.reportreply).delete(reply.deletereply);
