...
function solve(stringmatrix) {
    if (ifsolved(stringmatrix)) {
        ...
        const validmatrix = if_That_Is_Valid(possibilities);
...
function if_That_Is_Valid(matrix) {
    var sol = [];
    ...
    for (var i = 0; i < 9; i++) {
        ...
        for (var j = 0; j < 9; j++) {
            if (cur.includes(matrix[j][i])) {
                return false;
            } else if (matrix[j][i] != ".") cur.push(matrix[j][i]);
        }
    }
    return true;
} //checkcol
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    Array.from(htmlinputs).forEach((input) =>
...
try {
    module.exports = {
        ...
        if_That_Is_Valid /*check row col and box */ ,
        solve,
