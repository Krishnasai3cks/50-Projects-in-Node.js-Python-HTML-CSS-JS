...
suite('UnitTests', () => {
  ...
  suite('Function ____()', () => {
    test('Valid "1-9" characters', (done) => {
      ...
      input.forEach((ele,i) =>{
        assert.equal(Solver.checkvalidinput(ele), input[i]);
      })
      done();
    });
    ...
    test('Invalid characters (anything other than "1-9") are not accepted', (done) => {
      ...
      input.forEach((el, i) => {
        assert.strictEqual(Solver.checkvalidinput(el), false);
      });
      done();
    });
  });
  ...
  suite('Function ____()', () => {
    test('Parses a valid puzzle string into an object', done => {
      ...
      assert.equal(Solver.createstringmatrix(input), output)
      done();
    });
    ...
    test('Shows an error for puzzles that are not 81 numbers long', done => {
      ...
      assert.equal(Solver.checkvalidinput(shortStr),errorMsg);
      assert.equal(Solver.checkvalidinput(longStr),errorMsg);
      done();
    });
  });     
  ...
  suite('Function ____()', () => {
    ...
    test('Valid puzzles pass', done => {
      const input = '769235418851496372432178956174569283395842761628713549283657194516924837947381625';
          const a = Solver.checkvalidinput(input);
          const b = Solver.convertmattostr(Solver.solve(a));
      
      assert.equal(Solver.solvefunction(b), true)
      done();
    });
...
const a = Solver.checkvalidinput(input);
      ...
      assert.equal(Solver.solvefunction(b), false)
      done();
    });
  });
  ...
  suite('Function ____()', () => {
    ...
    test('Returns the expected solution for an incomplete puzzle', done => {
      ...
      assert.equal(Solver.solvefunction(b), solution)
      done();
    });
  });
});
