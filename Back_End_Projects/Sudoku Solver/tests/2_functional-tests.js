...
suite('Functional Tests', () => {
  ...
  suite('Text area and sudoku grid update automatically', () => {
    ...
    test('Valid number in text area populates correct cell in grid', done => {
      ...
      textArea.value = '123';
      Solver.createstringmatrix(textArea.value);
  ...
  suite('Clear and solve buttons', () => {
    ...
    test('Function clearInput()', done => {
      ...
      Solver.createstringmatrix(puzzleStr);
      
      // Invoke function and grab array of grid values to test
      Solver.clearinput();
      const gridValues = Array.from(document.querySelectorAll('.sudoku-input')).map(cell => cell).filter(cell => (cell.value!='.')? cell.vaue: undefined);
    ...
    test('Function showSolution(solve(input))', done => {
      ...
      Solver.solvefunction(Solver.solve(puzzleStr));
