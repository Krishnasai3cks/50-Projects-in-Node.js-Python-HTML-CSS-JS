...
med_dict = {1: 0, 2: 1, 3: 1}
df['cholesterol'] = df['cholesterol'].map(med_dict)
df['gluc'] = df['gluc'].map(med_dict)
# Draw Categorical Plot


def draw_cat_plot():
    # Create DataFrame for cat plot using `pd.melt` using just the values from 'cholesterol', 'gluc', 'smoke', 'alco', 'active', and 'overweight'.
    df_cat = pd.melt(frame=df, value_vars=[
    ...
    sns.catplot(x='variable', y='total', hue='value',
                data=df_cat, col='cardio', kind='bar')
...
def draw_heat_map():
    ...
    sns.heatmap(corr, annot=True, fmt='.1f', mask=mask, vmax=.3,
